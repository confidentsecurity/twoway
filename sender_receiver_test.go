...
func TestSenderReceiver(t *testing.T) {
	const requestOverhead = 7 + 65 // header bytes (7) + encap key (65)
	...
	for name, tc := range test.SendReceiveTests() {
		t.Run(name+", unchunked", func(t *testing.T) {
			...
			sender, err := twoway.NewRequestSender(suite, keyID, pubKey, rand.Reader)
			require.NoError(t, err)
			...
			require.Equal(t, tc.UnchunkedLen+requestOverhead, reqCTBuf.Len())

			// now pretend we're on the receiving end.
			receiver, err := twoway.NewRequestReceiver(suite, keyID, privKey, rand.Reader)
			require.NoError(t, err)
		...
		t.Run(name+", chunked", func(t *testing.T) {
			...
			// this sealer includes a header in its output.
			require.Equal(t, tc.ChunkedLen+requestOverhead, reqCTBuf.Len())
